"""Wrap abliterated text-only Qwen3_5ForCausalLM into the multimodal
Qwen3_5ForConditionalGeneration so sglang can serve it. Env-driven SRC/DST."""
...
BASE = os.environ.get("BASE_MM", "Qwen/Qwen3.5-4B")
SRC = os.environ["SRC"]
DST = os.environ["DST"]

print(f"[load mm base] {BASE}", flush=True)
...
try:
    AutoProcessor.from_pretrained(BASE, trust_remote_code=True).save_pretrained(DST)
