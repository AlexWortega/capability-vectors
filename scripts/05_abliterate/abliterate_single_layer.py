"""Abliterate text-only Qwen3.5-4B (soyuz_merged) along PASS-vs-FAIL direction.

Loads direction from /workspace/capvec_pf/vectors/pf_dir_L<N>.pt
Orthogonalizes:
  - embed_tokens rows
  - per-layer self_attn.o_proj.linear cols (Gemma4ClippableLinear? No — Qwen has plain Linear)
  - per-layer mlp.down_proj.linear cols
Saves to /workspace/abliterated_soyuz/
"""
...
MODEL_PATH = "/workspace/ckpts_qwen35_soyuz/clawd_soyuz_merged"  # text-only soyuz merged
...
def main():
    ...
    ap.add_argument("--dir-pt", default="/workspace/capvec_pf/vectors/pf_dir_L11.pt")
    ap.add_argument("--out", default="/workspace/abliterated_soyuz")
    ...
    L = blob["layer"]
    r = blob["dir"].float()
    print(f"using direction L={L} AUC={blob.get('auc',0):.3f} margin={blob.get('margin',0):+.3f}", flush=True)
    ...
    meta = {"source": MODEL_PATH, "direction_pt": args.dir_pt, "layer": L,
            "auc": float(blob.get("auc", 0)), "margin": float(blob.get("margin", 0)),
            "strength": args.strength, "method": "weight-ortho FAIL-direction removal"}
