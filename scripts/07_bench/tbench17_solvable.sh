...
OUT_ROOT="$RUN_DIR/tbench2_soyuz/$(date -u +%Y%m%d_%H%M%S)-solvable17"
NETWORK=$(docker inspect sglang-soyuz -f '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}')
RUNNER="$RUN_DIR/terminus_runner.py"
PARALLEL=${PARALLEL:-2}
MODEL_TAG="soyuz"
BASE_URL="http://sglang-soyuz:30014/v1"

# Union of all tasks solved by at least one prior model (ckpt600/clawd-100/-200/-rft/-rift)
TASKS=(
    build-pmars
    cobol-modernization
...
echo "[init] N=${#TASKS[@]} parallel=$PARALLEL model=$MODEL_TAG base=$BASE_URL"
...
echo "soyuz PASS $PASSES / $TOTAL on union-of-solvable-17"
